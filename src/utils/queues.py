...
def pack_int(_int):
    return struct.pack("!i", _int)


def unpack_int(_bytes):
...
def main():
    ...
    for (qname, qfactory), (name, num, data) in product(queues, tests):
        basedir = tempfile.mkdtemp(prefix="pqueue-%s-%s-" % (qname, name,), dir='/tmp/')
        try:
...
# def test():
#
#     address_jobs = ('127.0.0.1', 8506)
#     address_results = ('127.0.0.1', 8507)
#     address_shared = ('127.0.0.1', 8508)
#     auth = 'prova_test'
#
#     server_jobs = QueueManager(address=address_jobs[0],
#                                port=address_jobs[1],
#                                authkey=auth,
#                                queues={'tasks_queue': Queue()})
#     j = start_server(server_jobs)
#
#     server_results = QueueManager(address=address_results[0],
#                                   port=address_results[1],
#                                   authkey=auth,
#                                   queues={'results_queue': Queue()})
#     r = start_server(server_results)
#
#     share_server = SharedResultsManager(address=address_shared[0],
#                                         port=address_shared[1],
#                                         authkey=auth)
#     s = start_server(share_server)
#
#     client_jobs = ClientManager(address=(address_jobs[0], address_jobs[1]),
#                                 authkey=auth,
#                                 queues=['tasks_queue'])
#
#     client_results = ClientManager(address=(address_results[0], address_results[1]),
#                                    authkey=auth,
#                                    queues=['results_queue'])
#
#     client_shared = ClientManager(address=(address_shared[0], address_shared[1]),
#                                   authkey=auth,
#                                   queues=['results'])
#
#     tasks = client_jobs.get_tasks_queue()
#     results = client_results.get_results_queue()
#     shared = client_shared.get_results()
#
#     num_consumers = cpu_count() / 2
#     num_jobs = 100
#     print('Creating %d consumers' % num_consumers)
#     # Switch between results and shared
#     consumers = [Worker(tasks, shared, wait=False, poison_pill='-STOP-')
#                  for i in range(num_consumers)]
#
#     print('Submitting tasks')
#
#     for n in range(num_jobs):
#         tasks.put(Task(uuid.uuid4(), print_func, n))
#
...
#             print(k, shared[k])
#
#         print("Total results: ", len(shared))
#
#     else:
#         counter = 0
